...
class InventoryDashboardView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["search_query"] = (
            search_query  # Pass search_query to the template to reuse it in the search form
        )
...
def search_skus(request):
    ...
    skus = SKU.objects.filter(name__icontains=query).values("name")[
        :5
    ]  # Limit results to 5 for example
...
class SKUDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if sku.supplier:
            print(
                "Supplier Name:", sku.supplier.supplier_name
            )  # Use supplier_name instead of name
...
class DeleteSKUView(DeleteView):
    ...
    success_url = reverse_lazy(
        "dashboard"
    )  # Assuming 'dashboard' is your dashboard view's URL name
...
def perform_sku_deletion(request):
    if request.method == "POST":
        ...
        if sku_ids:
            ...
            messages.success(
                request, f"Selected SKUs have been soft deleted successfully."
            )
        else:
