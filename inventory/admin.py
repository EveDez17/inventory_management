    ...
    def get_search_results(self, request, queryset, search_term):
        ...
        if search_term:
            queryset |= self.model.objects.filter(
                supplier__supplier_name__icontains=search_term
            )
...
class SupplierAdmin(admin.ModelAdmin):
    ...
    list_skus.short_description = (
        "SKUs"  # Sets a header for the column in the admin list view
    )
...
class AddressAdmin(admin.ModelAdmin):
    ...
    list_filter = (
        ...
        "county",
        "city",
    )  # Allows filtering by these fields in the admin
    search_fields = (
        "street_name",
        "city",
        "post_code",
        "county",
        "country",
    )  # Enables a search box for these fields

    # Optional: Customize the form layout in the admin detail view
    fieldsets = (
        (None, {"fields": ("street_number", "street_name", "city", "county")}),
        (
            "Advanced options",
            {
                "classes": ("collapse",),
